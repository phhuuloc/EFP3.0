...
const int Kripke::get_id_const() const
{
	return pId;
}
...
bool Kripke::entail_BForm(BFNode node, Agent ag, bool atleastone) const
{
    ...
    for (auto itTemp = states_list.begin(); itTemp != states_list.end(); itTemp++){
        if (itTemp->get_id_const() == pId){
            pState = *itTemp;
            break;
        }
    }
...
{
    States temp = states_list;
    Kstate src;// = states_list.at(fromStateID);
    States::iterator itState;
    for (itState = temp.begin(); itState != temp.end(); itState++){
        if ((*itState).get_id() == fromStateID){
            src = *itState;
            break;
        }
    }
...
void Kripke::add_edge(Kedge k)
{
    ...
    States::iterator itTemp;
    Kstate* stFrom;// = &states_list.at(k.get_from());
    Kstate* stTo;// = &states_list.at(k.get_to());
    for (itTemp = states_list.begin(); itTemp != states_list.end(); itTemp++){
        if (itTemp->get_id() == k.get_from())
            stFrom = &(*itTemp);
        if (itTemp->get_id() == k.get_to())
            stTo = &(*itTemp);
    }
