...
KAction::KAction(const Planner* planner, const MyAction act, const PropositionType proptype)
{
  m_planner = planner;
  m_act = act;
  m_type = proptype;
...
void KAction::set_name(const MyAction act) {
  m_act = act;
}
...
Planner KAction::get_planner() const
{
  return *m_planner;
}
...
const MyAction KAction::get_act() const
{
  return m_act;
}
