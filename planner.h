...
public:
    ...
    int maxSubGoal();//return the maximal subgoal this state satisfy
    int loopInRealWorld();//return the maximal loop in real world
};
...
class Priority_Queue
{
    ...
    public:
        ...
        void insert(CKripke* item, bool debug)
        {
            ...
            int priority = item->maxSubGoal();
            int sub_priority = item->loopInRealWorld();
            int current_lenght = item->get_path().size();
            if(debug){
            ...
            tmp->sub_priority = sub_priority;      

            // if (front == NULL || priority > front->priority || (priority == front->priority && sub_priority > front->sub_priority) || (sub_priority == front->sub_priority && front->ck->get_path().size() > current_lenght)){
            //     tmp->link = front;
            //     front = tmp;
            // }  else{
            //     q = front;
            //     while (q->link != NULL && q->link->priority >= priority && q->link->sub_priority >= sub_priority && q->link->ck->get_path().size() <= current_lenght){
            //         q=q->link;
            //     }
            //     tmp->link = q->link;
            //     q->link = tmp;
            ...
            if (front == NULL || priority > front->priority || (priority == front->priority && front->ck->get_path().size() > current_lenght))
            {
                tmp->link = front;
                front = tmp;
            }
            ...
            {   
                q = front;
                while (q->link != NULL && q->link->priority >= priority && q->link->ck->get_path().size() <= current_lenght){
                    q=q->link;
                }
                tmp->link = q->link;
                q->link = tmp;
...
protected:
    ...
    AgentSet get_executor(const KAction&) const;
...
//   KAction* add_ontic(const string str, PropositionType);
  KAction* add_ontic(const MyAction act, PropositionType);
  ...
  Kripke* cross_product(Kripke,UpdateModel);
